...
const WhatWeDoSection = () => {
  ...
  return (
    <section id="what-we-do" ref={sectionRef} className="min-h-screen py-20 px-8 md:px-16 bg-cream text-charcoal flex flex-col justify-center items-center">
      <div className={`transition-all duration-1000 ease-in-out ${isSectionVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <p className="font-sans text-lg md:text-xl text-center mb-16 max-w-2xl mx-auto text-charcoal/80">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12 w-full max-w-5xl">
        {services.map((service, index) => {
          ...
          return (
            ...
            >
              <div className="mb-6 w-16 h-16 relative text-gold">
                <Image src={service.icon} alt={`${service.name} icon`} layout="fill" className="filter-gold" />
              </div>
              <h3 className="font-display text-2xl font-semibold mb-3 text-charcoal">{service.name}</h3>
              <p className="font-sans text-charcoal/70 text-sm leading-relaxed">{service.copy}</p>
            </div>
          );
        })}
      </div>
